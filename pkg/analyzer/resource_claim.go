...
func (ResourceClaimAnalyzer) Analyze(a common.Analyzer) ([]common.Result, error) {
	...
	// DeviceClass is cluster-scoped. Do not apply claim name or label selectors
	// here: they limit which claims are analyzed, but must not hide a referenced
	...
	claims, err := client.ResourceClaims(a.Namespace).List(a.Context, a.ListOptions())
	if err != nil {
		return nil, err
	}
