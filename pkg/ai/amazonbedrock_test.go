...
func TestBedrockInferenceProfileARN(t *testing.T) {
	...
	// Skip the test if we're in CI environment since we can't mock AWS services easily
...
func TestExtractModelFromInferenceProfile(t *testing.T) {
	client := &AmazonBedRockClient{models: testModels}
	
	// Create a mock inference profile output
	modelARN := "arn:aws:bedrock:us-east-1::foundation-model/anthropic.claude-3-7-sonnet-20250219-v1:0"
	profile := &bedrock.GetInferenceProfileOutput{
	...
	// Test extracting model ID from the profile
	modelID, err := client.extractModelFromInferenceProfile(profile)
	assert.Nil(t, err, "Error should be nil")
	assert.Equal(t, "anthropic.claude-3-7-sonnet-20250219-v1:0", modelID, "Model ID should match")
	
	// Test with empty models array
	emptyProfile := &bedrock.GetInferenceProfileOutput{
		Models: []*bedrock.ModelSummary{},
	}
	
	_, err = client.extractModelFromInferenceProfile(emptyProfile)
	assert.NotNil(t, err, "Error should not be nil for empty models array")
	
	// Test with nil profile
	_, err = client.extractModelFromInferenceProfile(nil)
	assert.NotNil(t, err, "Error should not be nil for nil profile")
	
	// Test with invalid model ARN format
	invalidModelARN := "invalid-arn"
	invalidProfile := &bedrock.GetInferenceProfileOutput{
		Models: []*bedrock.ModelSummary{
			{
				ModelArn: &invalidModelARN,
			},
		},
	}
	
	_, err = client.extractModelFromInferenceProfile(invalidProfile)
	assert.NotNil(t, err, "Error should not be nil for invalid model ARN format")
}
